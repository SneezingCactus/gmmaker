window.smInjectBonkScript = function(){
	fetch("js/alpha2s.js?smEntryPoint").then((e) => {
	e.text().then((code) => {
		console.log("%c[SneezingMod] Injecting alpha2s.js...", "background: black; color: #42f563;");
		window.bonkSrc = code;
		fetch("https://bonkleagues.io/api/client-init").then((function(e) {
            return e.json();
        })).then((function(e) {
			// my own regexes
			e.regex.funcs.push({name: "Box2D", regex: "requirejs\\(\\[[^\\]]+\\],function\\([^,]+,[^,]+,([^)]+)", isConstructor: false})
			e.regex.funcs.push({name: "CurrentGameState", regex: "];}};([A-Z](\\[[A-Za-z0-9]+(\\[[0-9]+]){2}]){2})", isConstructor: false})
			//e.regex.funcs.push({name: "MusicManager", regex: "l:[A-Za-z0-9]+\\[[0-9]+]};};function ([A-Za-z0-9]+)", isConstructor: true})
			//e.regex.funcs.push({name: "CollisionHandler", regex: "j7S\\[8]\\[0]];(P)", isConstructor: true})
			//e.regex.funcs.push({name: "BonkGraphicsFuncs", regex: "};function ([A-Za-z0-9]+)[A-Za-z0-9(),]+{\"use strict\"", isConstructor: true})
			//e.regex.funcs.push({name: "GameRendererClass", regex: "null;[A-Za-z0-9\\[\\]]+\\(true\\);[A-Za-z0-9\\[\\]]+=null;}};([A-Za-z0-9\\[\\]]+)=class", isConstructor: true})
			//e.regex.funcs.push({name: "BonkUtils", regex: "(...\\[[0-9]+])={};function [A-Za-z0-9]+\\([A-Za-z0-9, ]+\\){\"use strict\"", isConstructor: true})

			// inject alpha2s
			let newBonkSrc = bonkSrc;
			let funcHooks = "";
			e.regex.funcs.map((function(func){
				if(func.name == "MapColorPicker") return func;
			...
			newBonkSrc = newBonkSrc.replace(new RegExp(e.regex.inject.regex), `${e.regex.inject.wrap.left}${funcHooks}window.initSM();${e.regex.inject.wrap.right}`);
			newBonkSrc = newBonkSrc.replace(new RegExp(e.regex.vars.args, "g"), "\nvar $1 = [arguments]; window.smBonkVars.$1 = () => $1\n\n")
			window.smBonkVars = {};

			// init the now modified alpha2s
			createnewscript(newBonkSrc);
		}))
	})
})
}
...
window.smInjectOtherBonkVars = function(){
	for(let i = 0, vars = Object.keys(window.smBonkVars); i != vars.length; i++)
		for(let i2 = 0, varChildren = window.smBonkVars[vars[i]](); i2 != vars.length; i2++)
...
function createnewscriptwithurl(url){
	var s = document.createElement('script');
    s.type = 'text/javascript';
	s.async = false;
	s.src = url;
...
// gotta keep this somewhere
function deobfBonk(){
			window.bonkSrc_DEOBF = window.bonkSrc;
			let temp = window.bonkSrc;
			console.log("Replacing references to S8TT.U7S...")
			re = /n4V\.[KW]7S\(([0-9]+)\);(var |)([A-Za-z0-9[\]]+)=n4V\.[UF]7S\(([0-9,]+)\);/gm;
			while (r = re.exec(temp)){
				window.bonkSrc_DEOBF = window.bonkSrc_DEOBF.replace(r[0],"");
				S8TT.W7S(parseInt(r[1]));
				window.bonkSrc_DEOBF = window.bonkSrc_DEOBF.split(r[3]).join(S8TT.U7S(...r[4].split(',').map(Number)))
			}
			temp = window.bonkSrc_DEOBF;
			console.log("Replacing references to E8TT...")
			re = /([a-zA-Z0-9\[\]]+)=E8TT/gm;
			while (r = re.exec(temp)){
				window.bonkSrc_DEOBF = window.bonkSrc_DEOBF.split(r[1]).join("E8TT");
			}
			temp = window.bonkSrc_DEOBF;
			re = /E8TT\[([0-9\[]+)\]/gm;
			while (r = re.exec(temp)){
				window.bonkSrc_DEOBF = window.bonkSrc_DEOBF.split(r[0]).join('"' + E8TT[parseInt(r[1])] + '"');
			}
			temp = window.bonkSrc_DEOBF;
			console.log("Replacing references to S8TT.M18...")
			re = /(S8TT|n4V)\.[MV]18\(([0-9\[]+)\)/gm;
			while (r = re.exec(temp)){
				window.bonkSrc_DEOBF = window.bonkSrc_DEOBF.split(r[0]).join('"' + S8TT.M18(parseInt(r[2])).replace(/\n/g, "\\n").replace(/\r/g, "\\r").replace(/\"/g, '\\"') + '"');
			}
			window.bonkSrc_DEOBF = window.bonkSrc_DEOBF.replace(/\["([^"]+)"\]/gm, '.$1');
			window.bonkSrc_DEOBF = window.bonkSrc_DEOBF.split("E8TT=E8TT;").join("")
			console.log(window.bonkSrc_DEOBF)
}
function deobfNewBonk(){
			window.bonkSrc_DEOBF = window.bonkSrc;
			let temp = window.bonkSrc;
			console.log("Replacing references to S8TT.U7S...")
			re = /W9h\.[uW]8C\(([0-9]+)\);(var |)([A-Za-z0-9[\]]+)=W9h\.[yZ]8C\(([0-9,]+)\);/gm;
			while (r = re.exec(temp)){
				window.bonkSrc_DEOBF = window.bonkSrc_DEOBF.replace(r[0],"");
				S8TT.W7S(parseInt(r[1]));
				window.bonkSrc_DEOBF = window.bonkSrc_DEOBF.split(r[3]).join(S8TT.U7S(...r[4].split(',').map(Number)))
			...
			temp = window.bonkSrc_DEOBF;
			console.log("Replacing references to T2vv...")
			re = /([a-zA-Z0-9\[\]]+)=T2vv/gm;
			while (r = re.exec(temp)){
				window.bonkSrc_DEOBF = window.bonkSrc_DEOBF.split(r[1]).join("T2vv");
			}
			temp = window.bonkSrc_DEOBF;
			re = /T2vv\[([0-9\[]+)\]/gm;
			while (r = re.exec(temp)){
			...
			temp = window.bonkSrc_DEOBF;
			console.log("Replacing references to S8TT.M18...")
			re = /(m2vv|W9h)\.[wk]1L\(([0-9\[]+)\)/gm;
			while (r = re.exec(temp)){
				window.bonkSrc_DEOBF = window.bonkSrc_DEOBF.split(r[0]).join('"' + S8TT.M18(parseInt(r[2])).replace(/\n/g, "\\n").replace(/\r/g, "\\r").replace(/\"/g, '\\"') + '"');
			}
			window.bonkSrc_DEOBF = window.bonkSrc_DEOBF.replace(/\["([^"]+)"\]/gm, '.$1');
			window.bonkSrc_DEOBF = window.bonkSrc_DEOBF.split("T2vv=T2vv;").join("")
			console.log(window.bonkSrc_DEOBF)
}